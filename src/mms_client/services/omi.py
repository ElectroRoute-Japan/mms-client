...
class OMIClient:
    ...
    def __init__(
        ...
        """Create a new MMS OMI client with the given participant, user, client type, and authentication.
        ...
        wrapper = ZWrapper(client_type, Interface.OMI, cert.to_adapter(), True, test)

        # Next, create a new signer for the given certificate
        signer = CryptoWrapper(cert)

        # Now, create a new serializer for the OMI schema
        serializer = Serializer(SchemaType.OMI, "MarketData")
